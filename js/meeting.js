...
ws.onmessage = async (message) => {
  ...
  switch (data.type) {
    case "room-role":
      ...
      console.log(`🚦 Role assigned: ${isInitiator ? "Initiator (will offer)" : "Waiter (will wait)"}`);
      break;
...
async function createOffer(user, attempt = 1) {
  const peer = await createPeer(user);
  const offer = await peer.createOffer();
  await peer.setLocalDescription(offer);
  ws.send(JSON.stringify({ type: "offer", offer, room, user: name, to: user }));

  console.log(`📨 Offer sent to ${user} (attempt ${attempt})`);

  // Retry sending offer if no answer received within 1 second, max 3 attempts
  setTimeout(() => {
    if (peers[user] && peers[user].signalingState === "have-local-offer" && attempt < 3) {
      console.log(`🔄 Retrying offer to ${user} (attempt ${attempt + 1})`);
      createOffer(user, attempt + 1);
    }
  }, 1000);
}
